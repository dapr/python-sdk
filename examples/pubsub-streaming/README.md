...
```bash
# 1. Start Subscriber
dapr run --app-id pubsub-streaming-subscriber --app-protocol grpc -- python3 subscriber.py  --topic=TOPIC_A1
```
...
```bash
# 2. Start Publisher
dapr run --app-id pubsub-streaming-publisher --app-protocol grpc --dapr-grpc-port=56011 --enable-app-health-check -- python3 publisher.py --topic=TOPIC_A1
```
...
```bash
# 1. Start Subscriber
dapr run --app-id pubsub-streaming-subscriber --app-protocol grpc -- python3 subscriber-handler.py  --topic=TOPIC_A2
```
...
```bash
# 2. Start Publisher
dapr run --app-id pubsub-streaming-publisher --app-protocol grpc --dapr-grpc-port=56011 --enable-app-health-check -- python3 publisher.py  --topic=TOPIC_A2
```
