...
map_state_change_kind_to_operation = {
...
class StateProvider(Generic[T]):
    def __init__(self, actor_client: DaprActorClientBase, state_serializer: Serializer=None):
        self._state_client = actor_client
        self._state_serializer = state_serializer or DefaultJSONSerializer()

    async def try_load_state(self, actor_type: str, actor_id: str, state_name: str) -> Tuple[bool, T]:
    ...
    async def save_state(self, actor_type: str, actor_id: str, state_changes: List[ActorStateChange]) -> None:
        """
        ...
        for state in state_changes:
            operations.append({
                "operation": map_state_change_kind_to_operation.get(state.change_kind) or '',
                "request": {
