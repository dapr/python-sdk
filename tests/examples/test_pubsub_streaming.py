...
@pytest.mark.example_dir('pubsub-streaming')
def test_pubsub_streaming(dapr):
    ...
    publisher_output = dapr.run(
        ...
        '--enable-app-health-check -- python3 publisher.py --topic=TOPIC_A1',
        timeout=30,
    )
...
def test_pubsub_streaming_handler(dapr):
    ...
    publisher_output = dapr.run(
        '--app-id python-publisher --app-protocol grpc --dapr-grpc-port=3500 '
        '--enable-app-health-check -- python3 publisher.py --topic=TOPIC_A2',
        timeout=30,
    )
    for line in EXPECTED_PUBLISHER:
        assert line in publisher_output, f'Missing in publisher output: {line}'
