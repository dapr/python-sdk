...
from dapr.ext.grpc.aio import App, Rule, InvokeMethodRequest, BindingRequest
...
class AppTests(unittest.TestCase):
    ...
    def test_subscribe_decorator(self):
        ...
        async def handle_default(event: v1.Event) -> None:
            pass

        @self._app.subscribe(
            pubsub_name='pubsub', topic='topic', rule=Rule('event.type == "test"', 1)
        )
        async def handle_test_event(event: v1.Event) -> None:
            pass

        @self._app.subscribe(pubsub_name='pubsub', topic='topic2', dead_letter_topic='topic2_dead')
        async def handle_dead_letter(event: v1.Event) -> None:
            pass
