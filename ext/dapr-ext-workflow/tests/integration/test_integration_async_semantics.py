...
@skip_integration
def test_integration_activity_attempt_exposed():
    # Verify that activity ctx exposes attempt via pass-through property
    runtime = WorkflowRuntime()

    @runtime.activity(name='probe_attempt_activity')
    def probe_attempt_activity(ctx, _=None):
        attempt = getattr(ctx, 'attempt', None)
        return {'attempt': attempt}

    @runtime.async_workflow(name='activity_attempt_wf')
    async def activity_attempt_wf(ctx: AsyncWorkflowContext):
        return await ctx.call_activity(probe_attempt_activity, input=None)

    runtime.start()
    try:
        time.sleep(2)
        client = DaprWorkflowClient()
        iid = f'act-attempt-{int(time.time() * 1000)}'
        client.schedule_new_workflow(workflow=activity_attempt_wf, instance_id=iid)
        client.wait_for_workflow_start(iid, timeout_in_seconds=30)
        st = client.wait_for_workflow_completion(iid, timeout_in_seconds=60)
        assert st is not None and st.runtime_status.name == 'COMPLETED'
        import json as _json
...
def test_integration_async_contextvars_trace_propagation(monkeypatch):
    ...
    def cv_flaky_probe(ctx, _=None):
        ...
        if attempt is not None and attempt == 0:
            # Fail first attempt (when engine exposes attempt) to trigger retry
        ...
        try:
            ...
            if tok is not None:
                current_trace.reset(tok)
        after = current_trace.get()
        return {'before': before, 'inner': inner, 'after': after, 'attempt': attempt}
    ...
    async def cv_parent(ctx: AsyncWorkflowContext, _=None):
        ...
        act_retry = await ctx.call_activity(
            ...
            retry_policy=RetryPolicy(
                first_retry_interval=timedelta(seconds=0), max_number_of_attempts=2
            ),
        )
        return {
            'before': top_before,
    ...
    try:
        ...
        # Flaky activity retried: second attempt returns with attempt >= 1 and parent context
        act_retry = out.get('act_retry') or {}
        att = act_retry.get('attempt')
        if att is not None:
            assert att in (1, 2)
