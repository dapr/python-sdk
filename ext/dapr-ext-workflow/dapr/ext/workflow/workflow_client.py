...
class WorkflowClient:
    ...
    def schedule_new_workflow(self, workflow: Workflow, *,
        ...
        return self._obj.schedule_new_orchestration(workflow.__name__, version=version, input=input, instance_id=instance_id, start_at=start_at)

    def get_workflow_state(self, instance_id: str, *, fetch_payloads: bool = True) -> Union[WorkflowState, None]:
    ...
    def wait_for_workflow_start(self, instance_id: str, *,
        ...
        state = self._obj.wait_for_orchestration_start(instance_id, fetch_payloads=fetch_payloads, timeout=timeout)
        return WorkflowState(state) if state else None
    
    def wait_for_workflow_completion(self, instance_id: str, *,
                                          fetch_payloads: bool = True,
                                          timeout: int = 60) -> Union[WorkflowState, None]:
        state = self._obj.wait_for_orchestration_completion(instance_id, fetch_payloads=fetch_payloads, timeout=timeout)
        return WorkflowState(state) if state else None