...
class StrandsMapper(BaseAgentMapper):
    ...
    def _extract_from_strands_agent(self, agent: Any) -> Dict[str, Any]:
        ...
        if model:
            ...
            model_id = getattr(model, 'model_id', None) or getattr(model, 'model', None) or getattr(model, 'name', None)
            if config:
        ...
        if tool_registry and hasattr(tool_registry, 'registry'):
            ...
            if isinstance(registry_dict, dict):
                for tool_name, agent_tool in registry_dict.items():
                    tool_info = {
                        'name': tool_name,
                        'description': getattr(agent_tool, 'description', '') or getattr(agent_tool, '__doc__', ''),
                    }
    ...
    def _get_session_agent(self, session_manager: "DaprSessionManager", session_id: str) -> Optional["SessionAgent"]:
        """
        ...
        try:
            ...
            if not manifest or 'agents' not in manifest or not manifest['agents']:
                logger.warning(f"No agents found in session {session_id} - session may not have agents yet. Use fallback metadata.")
                return None
            ...
            if session_agent:
                logger.info(f"Successfully loaded SessionAgent '{agent_id}' with state keys: {list(session_agent.state.keys()) if session_agent.state else []}")
            else:
            ...
            logger.error(f"Failed to read SessionAgent from session {session_id}: {e}", exc_info=True)
            return None
    ...
    def _extract_tools_metadata(self, agent_state: Dict[str, Any]) -> List[ToolMetadata]:
        ...
        for tool in tools_list:
            if isinstance(tool, dict):
                tool_metadata_list.append(ToolMetadata(
        ...
        if self._is_strands_agent(agent):
            ...
            if llm_info:
                llm_metadata = LLMMetadata(
                    client=llm_info.get('model_type', 'unknown'),  # OpenAIModel, AnthropicModel, etc.
            ...
            for tool in tools_list:
                if isinstance(tool, dict):
                    tools_metadata.append(ToolMetadata(
            ...
            return AgentMetadataSchema(
                ...
                agent=AgentMetadata(
                    ...
                    instructions=extracted.get('instructions') if extracted.get('instructions') else None,
                ...
                tool_choice="auto" if tools_metadata else None,  # Set tool_choice based on whether tools exist
