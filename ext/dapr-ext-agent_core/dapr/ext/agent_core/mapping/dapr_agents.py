    ...
    def map_agent_metadata(self, agent: Any, schema_version: str) -> AgentMetadataSchema:
        ...
        return AgentMetadataSchema(
            ...
            llm=LLMMetadata(
                ...
                prompt_template=type(getattr(llm, "prompt_template", None)).__name__ if llm and getattr(llm, "prompt_template", None) else None,
            ),
            registry=RegistryMetadata(
                statestore=getattr(getattr(registry, "store", None), "store_name", None) if registry else None,
